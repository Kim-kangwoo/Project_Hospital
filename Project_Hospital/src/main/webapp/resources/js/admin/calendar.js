...
function showDetailReserve(date) {
  ...
  if (
    !(
      selectDayData.amReservOtIdList.length === 0 &&
      selectDayData.pmReservOtIdList.length === 0
    )
  ) {
    // 오전, 오후 상태 휴진일 경우 return 처리
    if (
      selectDayData.amStatus == '휴진-진료없음' &&
      selectDayData.pmStatus == '휴진-진료없음'
    ) {
      ...
      if (selectDayData.amStatus != null && selectDayData.pmStatus != null) {
        if (
          selectDayData.amStatus == '정원초과' &&
          selectDayData.pmStatus == '정원초과'
        ) {
          alert(
...
function setStatus(content, index) {
  ...
  switch (content.amStatus) {
    case null:
      document.querySelectorAll(
        'select[class=amSelect' + index + '] option'
      )[0].selected = true;
      break;
  ...
  switch (content.pmStatus) {
    case null:
      document.querySelectorAll(
        'select[class=pmSelect' + index + '] option'
      )[0].selected = true;
      break;
...
function validateUpdate() {
  ...
  console.log('--- selectedAfterStatus ---');
  console.log(selectedAfterStatus);
  ...
  if (validate.size == 1 && validate.has('N')) {
    validateFlag = false;
  }
...
function selectedDayUpdate(day) {
  if (
    confirm('선택 시 해당 요일에 전체 적용되어집니다\n계속 진행하시겠습니까?')
  ) {
    // 선택된 값 추출 상태, 시간타입, 요일
