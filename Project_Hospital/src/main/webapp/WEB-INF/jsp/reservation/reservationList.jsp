...
<script>
	function reservationDelete(reservId, otId) {
		let result = confirm("예약을 취소하시겠습니까?");
		if(result) {
			location.href = "reservationDelete?reservId=" + reservId + "&otId=" + otId;
		}
	}
</script>
</head>
...
<div class="container">
	<div style="width: 1200px; margin: 20px auto 40px">
		<div class="myrs">
			<div class="myrs_now">진료예약현황</div>
			<div class="myrs_info">이름 : <span style="color: #92e; font-weight: bold;">${ userClientOne.userName }</span></div>
			<div class="myrs_info">환자번호 : <span style="color: #92e; font-weight: bold;">${ userClientOne.userNum }</span></div>
			<div class="myrs_info">생년월일 : <span style="color: #92e; font-weight: bold;">${ userClientOne.userBirth }</span></div>
			<div class="btn btn-primary scheduleModal" style="margin: 7px 0px 7px 290px;" id="${user.userNum}" data-bs-toggle="modal" data-bs-target="#scheduleModal">
				진료스케줄
			</div>
		</div>
		<div>
			<table class="myrs_table">
				<colgroup>
					<col width="10%">
					<col width="16%">
					<col width="16%">
				...
				<tbody>
					<c:set var="myrsCount" value="1"></c:set>
					<c:forEach var="reservation" items="${ reservationList }">
						<c:forEach var="openingTimeTablereservation" items="${ openingTimeTablereservationList }">
							<c:forEach var="hpMemberDept" items="${ hpMemberDeptList }">
								<c:if test="${ openingTimeTablereservation.otId == hpMemberDept.otId && hpMemberDept.otId == reservation.otId }">
									<tr>
										<td>${ myrsCount }</td>
										<td>${ openingTimeTablereservation.yearDate }</td>
										<td>${ openingTimeTablereservation.time }</td>
										<td>${ hpMemberDept.deptName }</td>
										<td>${ hpMemberDept.drName } ${ hpMemberDept.drPosition }</td>
										<td><input class="btn btn-primary" type="button" value="취소" onclick="reservationDelete('${ reservation.reservId }', '${ reservation.otId }')"></td>
									</tr>
									<c:set var="myrsCount" value="${ myrsCount + 1 }"></c:set>
								</c:if>
							</c:forEach>
						</c:forEach>
					</c:forEach>
