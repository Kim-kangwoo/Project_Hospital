...
<head>
<meta charset="UTF-8">
<title>(관리자) 건강 정보 : 신체 분류 관리 페이지</title>
<script src="https://code.jquery.com/jquery-3.7.1.min.js" ></script>
</head>
<body>
	<h3>(관리자) 건강 정보 : 신체 분류 관리 페이지</h3>
	<div class="body_wrap">
	...
	<div class="content">
		<div class="top_bar bg_w">
			<a href="#" class="top_left health_con"><span>신체 분류 관리</span></a>
			<!-- 탑바 컨텐츠-->
		...
		<div class="tab03_wrap normal bg_w">
			...
			<div class="tab03_mid">
				...
				<div class="mid_btn">
					...
					<a href="insertbodyClassificationPage" class="bg_2 f_w500 f_s15">신체 분류 추가</a>
				</div>
			</div> <!-- Hihy 건강저장소 TV  --> 
			<table class="table_sty01">
				...
				<c:forEach var="body" items="${bodyClassificationList}">
					<tr>
						<td><input type="checkbox" class="chk" value="${body.bodyPartId}"></td>
						<td>${body.bodyPartName }</td>
						<td><a href="updateBodyPartNamePage?BodyPartId=${body.bodyPartId}"><span class="pencil">수정</span></a></td>
						<td>
							<div class="del_box">
								<a href="" class="deleteBtnClass" data-body-part-id="${body.bodyPartId}"><img src="${pageContext.request.contextPath }/resources/images/delete.png" alt="">삭제</a>
							</div>
						</td>
					</tr>
				</c:forEach>
...
<script>
	...
	document.querySelectorAll('.deleteBtnClass').forEach(btn => {
		btn.addEventListener('click', function(event) {
			...
			const bodyPartId = this.dataset.bodyPartId; 
			console.log("삭제 선택한 id : " + bodyPartId);
			
			if(confirm("정말 삭제하시겠습니까?")) {
				deleteAjax(bodyPartId);
			} else {
				return;
			}
