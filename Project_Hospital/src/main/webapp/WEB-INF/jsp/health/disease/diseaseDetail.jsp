...
<head>
<meta charset="UTF-8">
<title>질환정보 상세 페이지</title>
	<link rel="stylesheet" href="${pageContext.request.contextPath }/resources/minseo_css/diseaseSearch.css" />
	<link rel="stylesheet" href="${pageContext.request.contextPath }/resources/minseo_css/health/diseaseDetail.css" />
...
<div id="breadcrumb" class="light">
	...
	<c:set var="board" value="${diseaseContent }" />
	<article id="content">
		...
		<div class="content-body cms-search cms-print" id="cms-content">
			<div class="article-header">
				<div class="subject-area">
					<h3 class="subject">${board.title}</h3>
					<div class="article-info">
								<span class="item name">관리자</span>
						<span class="item date">${board.createdAt}</span>
						<span class="item hit"><strong>조회수</strong>${board.boardCount}</span>
					</div>
				</div>
			</div> 
		<div class="article-body">
			<div class="subtitle">
				<h3 id="title">${koreanTitle}</h3>
				<strong>${englishTitle}</strong>
			</div>
			
			<c:forEach var="content" items="${splitedContentMap }">
				<c:if test="${content.key % 2 != 0 }"> <%--홀수 : 소제목 --%>
					<span><strong>${content.value} </strong></span><br>
				</c:if>
				<c:if test="${content.key % 2 == 0 }"> <%--짝수 : 내용 --%>
					<span>${content.value}</span><br><br>
				</c:if>
			</c:forEach>
		</div>
		<!-- 더보기 버튼 -->
