...
public class HealthInfoAjaxRestController {
	...
	public List<ExaminationBoardDTO> searchByCalendarExaminationBoard(@RequestBody SearchDTO searchDTO) {
		System.out.println("아작스 검사, 치료 게시판 캘린더 검색 : " + searchDTO);
		return healthService.searchByCalendarExaminationBoard(searchDTO);
		
	}
	
	@GetMapping("/searchByTermExaminationBoard/{term}")
	public List<ExaminationBoardDTO> searchByTermExaminationBoard(@PathVariable String term) {
		System.out.println("아작스 검사, 치료 게시판 구간 검색 : " + term);
		return healthService.searchByTermExaminationBoard(term);
	}
